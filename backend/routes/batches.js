...
router.post("/", (req, res) => {
    pool.getConnection(function (err, connection) {
        ...
        if (batch.hasOwnProperty("Samples")) {
            ...
            connection.query("INSERT INTO Batch SET ?", batch,
                (err, result) => {
                    connection.release();
                    if (err) {
                        res.status(500).send({
...
router.put("/:batchId/samples", (req, res) => {
    pool.getConnection(function (err, connection) {
        ...
        let batch = req.body;
        connection.query(
            `UPDATE Batch SET ? WHERE BatchId = ${batchId}`, batch,
            (err, result) => {
                connection.release();
                if (err) {
                    res.status(500).send({
                        success: false,
                        message: err.message || "Error: Cannot update batch",
                    });
                } else {
        ...
        connection.query(
            ...
            (err, result) => {
                ...
                if (err) {
                    ...
                    res.status(500).send({
                        success: false,
                        message: err.message || "Error: Cannot delete batch",
                    });
